...
// The checks that the C HUF reader accepts our emitted weight descriptions and
// 1X/4X literal payloads live in the FFI surfaces where C-binding usage belongs
// (tests/cross_validation.rs roundtrips our output through libzstd; the
// compare_ffi bench tracks ratio). They are intentionally not src unit tests so
// the library crate never imports the C bindings.
