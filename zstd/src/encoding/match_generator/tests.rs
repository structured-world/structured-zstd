...
pub(crate) fn level22_block_ranges(data: &[u8]) -> Vec<(usize, usize)> {
    ...
    while cursor < data.len() {
        ...
        let candidate_len = remaining.min(super::super::cost_model::HC_BLOCKSIZE_MAX);
        let block_len = crate::encoding::frame_compressor::optimal_block_size(
            ...
            remaining,
            super::super::cost_model::HC_BLOCKSIZE_MAX,
            savings,
        )
        .min(candidate_len)
        .max(1);
        ...
        if cursor >= super::super::cost_model::HC_BLOCKSIZE_MAX {
            savings = 3;
        }
    }
    ranges
...
fn collect_level22_sequences_with_delimiters(data: &[u8]) -> Vec<(usize, usize, usize)> {
    let mut driver = MatchGeneratorDriver::new(super::super::cost_model::HC_BLOCKSIZE_MAX, 1);
