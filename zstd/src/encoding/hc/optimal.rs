        ...
        *searched_at = None;
        ...
        if $current_len >= min_match_len {
            ...
            if !candidates.is_empty() {
                ...
                let node0_price = BtMatcher::cached_lit_length_price::<ACCURATE_PRICE>(
                    profile,
                    $stats,
                    initial_litlen,
                    &mut ll_cache,
                ...
                ll0_price = BtMatcher::cached_lit_length_price::<ACCURATE_PRICE>(
                    profile,
                    $stats,
                    0,
                    &mut ll_cache,
                ...
                ll1_price = BtMatcher::cached_lit_length_price::<ACCURATE_PRICE>(
                    profile,
                    $stats,
                    1,
                    &mut ll_cache,
            ...
            if let Some(candidate) = candidates.last() {
                ...
                if longest_len > sufficient_len {
                    ...
                    let ml_price = BtMatcher::cached_match_length_price::<ACCURATE_PRICE>(
                        profile,
                        $stats,
                        longest_len,
                        &mut ml_cache,
        ...
        while !seed_forced_shortest_path && pos <= last_pos && pos <= frontier_limit {
            ...
            if prev_node_price != u32::MAX {
                ...
                let lit_price = {
                    ...
                    BtMatcher::cached_literal_price::<ACCURATE_PRICE>(
                        profile,
                        $stats,
                        $current[pos - 1],
                ...
                let ll_delta = BtMatcher::cached_lit_length_delta_price::<ACCURATE_PRICE>(
                    profile,
                    $stats,
                    lit_len,
                    &mut ll_cache,
                ...
                if lit_cost <= node_pos_price {
                        ...
                        && pos < $current_len
                    {
                        if ll1_price < ll0_price {
                            ...
                            let ll_delta_next =
                                BtMatcher::cached_lit_length_delta_price::<ACCURATE_PRICE>(
                                profile,
                                $stats,
                                lit_len + 1,
                                &mut ll_cache,
...
impl HcMatchGenerator {
    ...
    // Stays on the SSE4.2 umbrella: the DP body calls
    // `priceset_range_nonabort_sse41`, whose `_mm_min_epu32` is SSE4.1, so
    ...
    /// SSE2 twin of [`Self::collect_optimal_candidates_initialized_sse42`].
    /// The per-position pipeline is identical; only the enclosing umbrella
    /// differs, so the SSE2 tier keeps the same straight-line inlining.
    ///
    /// # Safety
