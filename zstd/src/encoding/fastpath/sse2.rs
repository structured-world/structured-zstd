//! x86/x86_64 SSE2 fastpath variant. Hot-path functions are marked
//! `#[target_feature(enable = "sse2")]` so the 128-bit vector ops inline
//! freely inside this module.
//!
//! Selected at runtime when AVX2/BMI2 are unavailable. SSE2 is part of the
//! x86_64 baseline, so this tier is always available there; on 32-bit x86 it
//! is gated by the runtime probe.
...
/// 16-byte SSE2 vector prefix-length probe.
///
/// # Safety
/// `lhs` / `rhs` must point to at least `max` initialized bytes. SSE2 must be
/// available, per the umbrella attribute on this fn.
...
/// SSE2 variant of `common_prefix_len_ptr`. Vector loop then the shared
/// scalar tail.
///
/// # Safety
...
/// SSE2 variant of `count_match_from_indices`. Same invariants as the
/// scalar variant.
///
/// # Safety
